...
destinationFolder = "/Users/asampat3090/Desktop/xkcd" # Full path to destination folder. Needs to exist
...
for i in xrange(1, nrComics + 1):
    ...
    xkcd_img = Image.open(img_full_name)
    ...
    if xkcd_img.size[0]>xkcd_img.size[1]:
        ...
        # temp_1.show()
        temp_1.save(img_full_name)
        # resize the image to fit the screen while maintaining the aspect ratio.
        new_to_old_ratio=float(800)/float(temp_1.size[1])
        if temp_1.size[0]*new_to_old_ratio>600:
            ...
            canvas_img.paste(temp_2,(0,int(float(800-float(temp_1.size[1]*600)/float(temp_1.size[0]))/float(2))))
            print 1
        else:
            temp_2=temp_1.resize((int(math.floor(float(temp_1.size[0])*new_to_old_ratio)),800))
            # temp_2.show()
            # now paste this temp2 to the 600x800 canvas
        ...
        if xkcd_img.size[1]*new_to_old_ratio>800:
    
            temp_2=xkcd_img.resize((int(math.floor(float(xkcd_img.size[0])*float(800))/float(xkcd_img.size[1])),800))
            #temp_2.show()
            # now paste this temp2 to the 600x800 canvas
            canvas_img.paste(temp_2,(int(float(600-float(xkcd_img.size[0]*800)/float(xkcd_img.size[1]))/float(2)),0))
            print 3
        else:
            ...
            print temp_2.size
            # now paste this temp2 to the 600x800 canvas
            canvas_img.paste(temp_2,(0,int(float(800-float(xkcd_img.size[1])*new_to_old_ratio)/float(2))))
            print 4
    ...
    pathz = "/Users/asampat3090/Desktop/xkcd/" + img_full_name
